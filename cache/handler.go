...
func NewHandler(client HentaiAtHomeClient) (handler *Handler, err error) {
	...
	handler = &Handler{
		...
		pendingRegister:  make([]HVFile, 0, 50),
	}
...
func (h *Handler) HVFile(fileid string, hit bool) (hvf *HVFile) {
	if !IsValidId(fileid) {
...
func (h *Handler) moveFileToCacheDir( /*file *os.File*/ from string, hvFile HVFile) (err error) {
	to := hvFile.LocalFilePath()
	// TODO
...
func (h *Handler) addFileToActiveCache(hvFile *HVFile) (err error) {
	h.Lock()
	defer h.Unlock()
	...
	if affected == 0 {
		...
		h.db.InsertCachedFile(lasthit, hvFile)
	}

	h.cacheCount++
	h.cacheSize += hvFile.Size()
	h.updateStats()
...
func (h *Handler) addPendingRegisterFile(hvFile HVFile) (err error) {
	h.Lock()
	defer h.Unlock()
	// We only register files <= 10 MB. Larger files are handled outside the H@H network.

	if (hvFile.Size() <= 10485760) && !Settings.isStaticRange(hvFile.Id()) {
		log.Debugf("Added %s to pendingRegister", hvFile)
		h.pendingRegister = append(h.pendingRegister, hvFile)
		...
		log.Debugf("Not registering file %s - in static range or larger than 10 MB", hvFile)
	}
	return
}
func (h *Handler) deleteFileFromCache(toRemove *HVFile) (err error) {
	h.Lock()
	defer h.Unlock()
	return h.deleteFileFromCacheNosync(toRemove)
}

func (h *Handler) deleteFileFromCacheNosync(toRemove *HVFile) (err error) {
	//try {
	err = h.db.DeleteCachedFile(toRemove.Id())
	if err != nil {
		return
	}
	h.cacheCount--
	...
	// TODO
	err = removeFile(toRemove.LocalFilePath())
	log.Info("CacheHandler: Deleted cached file %s", toRemove.Id())
	h.updateStats()
	return
}
...
func (h *Handler) _checkAndFreeDiskSpace(file *os.File, noServerDeleteNotify bool) (deleteNotify []*HVFile, err error) {
	if file == nil {
	...
	if bytesToFree > 0 {
		...
		for bytesToFree > 0 && h.cacheCount > 0 {
			//synchronized(sqlite) {
			ids, _, err := h.db.CachedFileSortOnLasthit(0, 20)
			if err != nil {
				//
			}
...
func (h *Handler) pruneOldFiles() (deleteNotify []*HVFile, err error) {
	h.Lock()
	defer h.Unlock()
...
*/
func (h *Handler) processBlacklist(blacklisted []string, noServerDeleteNotify bool) (deleteNotify []*HVFile, err error) {
